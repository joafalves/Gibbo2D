...
namespace Microsoft.Xna.Framework
{
    public class OpenTKGameWindow : GameWindow, IDisposable
    {
        ...
        /// <summary>
        /// Get or sets the OpenTK GameWindow to be used, allows getting the auto-created default one or setting a custom one (for example one auto-created for a GLControl for WinForms interop)
        /// </summary>
        public OpenTK.GameWindow Window //CHANGED FROM: internal OpenTK.GameWindow Window { get { return window; } }
        {
            get { return window; }
            set
            {
                ...
                if (window == value)
                    return; //already using this window
                if (window != null)
                {
                    //dispose the automatically created window, since going to replace with a custom one
                    Dispose(true);
                    window = null;
                }
                window = value;
